...
const VideoPlayer: React.FC<VideoPlayerProps> = ({
  ...
  // Size class for height, default fallback
  const videoSizeClass = typeof size === "string" ? size : "h-64";

  return (
    <div className="bg-gradient-to-br from-slate-900 to-slate-800 rounded-xl shadow-2xl p-6">
      {/* Video Display */}
      <div className={`relative bg-black rounded-lg overflow-hidden mb-4 shadow-lg ${videoSizeClass}`}>
        <video
          ref={videoRef}
          src={videoSources[currentVideoIndex]}
          className={`w-full object-cover ${videoSizeClass}`}
          loop={isRepeat}
